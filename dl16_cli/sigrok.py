...
def _run(arguments: list[str]) -> str:
    try:
        ...
        raise Dl16Error(f"cannot run sigrok-cli: {exc}") from exc
    if result.returncode:
        detail = result.stderr.strip() or result.stdout.strip() or f"exit status {result.returncode}"
        raise Dl16Error(f"sigrok-cli failed: {detail}")
...
def list_sigrok_decoders() -> str:
    ...
    if marker not in output:
        raise Dl16Error("sigrok-cli did not report its protocol decoder list")
...
def _validate_component(label: str, value: str) -> None:
    if not value or any(character in value for character in ":=,\n\r"):
        raise Dl16Error(f"invalid sigrok {label}: {value!r}")
...
def _parse_assignments(values: list[str], *, channels: bool) -> list[str]:
    ...
    for value in values:
        ...
        if len(parts) != 2 or not all(part.strip() for part in parts):
            ...
            raise Dl16Error(f"sigrok {kind} must use NAME=VALUE syntax")
        name, assigned = (part.strip() for part in parts)
        _validate_component("assignment name", name)
        if name in seen:
            raise Dl16Error(f"duplicate sigrok assignment: {name}")
        seen.add(name)
        if channels:
            try:
                channel = int(assigned, 10)
            except ValueError as exc:
                raise Dl16Error(f"sigrok channel must be 0..15, got {assigned!r}") from exc
            if not 0 <= channel <= 15:
                raise Dl16Error(f"sigrok channel must be 0..15, got {channel}")
            assigned = f"CH{channel}"
        elif any(character in assigned for character in ":\n\r"):
            raise Dl16Error(f"invalid sigrok option value: {assigned!r}")
...
def decode_with_sigrok(
    ...
    *,
    ...
    if not channel_assignments:
        raise Dl16Error("sigrok decode requires at least one --channel NAME=CH mapping")
    decoder_spec = ":".join((decoder, *channel_assignments, *option_assignments))
    annotation_spec = annotations or decoder
    with tempfile.TemporaryDirectory(prefix="dl16-sigrok-") as temporary:
